...
class State(object):
        ...
        for x in newp:
            ...
            for y in att:
                ...
                for k, v in action_list.items():
                    ...
                    ps_actANDs = self.get_ps_actANDs(
                        x, y, [k, v], index, title)
    ...
    def get_ps_actANDs(self, after, before, action, index, title):
        ...
        if (title[index][0] not in effect) or (
                title[index][1] not in effect[title[index][0]]):
            if before == after:
                return self._cond_satisfy
            else:
                return self._cond_notsatisfy
        ...
        if (title[index][1] in precond[title[index][0]]) and (
                precond[title[index][0]][title[index][1]] != before):
            return self._cond_notsatisfy
