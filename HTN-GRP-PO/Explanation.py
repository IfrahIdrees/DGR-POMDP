...
class Explanation(object):
    ...
    def __init__(self, v=0, forest=[], pendingSet=[], start_task={}):
        ...
        # format {task1: 0, task2:1}, 0 stands for not started yet, 1 stands
        # for ongoing
...
##########################################################################
    ...
    def generate_new_expla_part1(self, act_expla):
        ...
        for start_task in tempstart_task:
            # inside this explanation, "start_task" has not been started
            if tempstart_task[start_task] == 0:
                    ...
                    for g in newTaskNets:
                        if tempstart_task[g._goalName] == 0:
                            ...
                            if g._complete:
                                ...
                                newexp = Explanation(v=prob, forest=list(
                                    self._forest), start_task=newstart_task)
                            else:
                                newforest = list(self._forest)
                                # here means it will start a new goal.
                                # However, if the already started goal do not
                                # have any execute sequence, remove
                                for tasknet in newforest:
                                    if len(
                                            tasknet._execute_sequence._sequence) == 0:
                                        newstart_task[tasknet._goalName] = 0
                                        newforest.remove(tasknet)
                                newforest.append(g)
                                newstart_task[g._goalName] = 1
                                newexp = Explanation(
                                    v=prob, forest=newforest, start_task=newstart_task)
        ...
        for branch in method["precondition"]:
            ...
            for ob_name in branch:
                for attri in branch[ob_name]:
                    prob_temp = prob_temp * \
                        db.get_attribute_prob_1(branch[ob_name][attri], ob_name, attri)
    ...
    def my_create_new_node(self, parent, decompose, childTree):
        ...
        for x in decompose[1]:
            if x == known_child.tag:
                ...
                mydata = Node_data(
                    pre=decompose[1][x]["pre"],
                    dec=decompose[1][x]["dec"])
                newTree.create_node(x, x, parent=newTree.root, data=mydata)
        return [newTree, decompose[0]]


##########################################################################
#####               generate the pending set for each explanation based on
##########################################################################

    def create_pendingSet(self):
        if len(self._pendingSet) == 0:
            pending_set = self.real_create_pendingSet()
        else:
            pending_set = self.normalize_pendingSet_prior()
        self.set_pendingSet(pending_set)
        return pending_set
    ...
    def real_create_pendingSet(self):
        ...
        # if currently the pending set has no action, need to initialize from
        # start tasks
        if len(pendingSet) == 0:
        ...
        if len(self._forest) == 0:
            ...
            for taskNet in self._forest:
                for taskNetPending in taskNet._pendingset:
                    ...
                    # only select nodes whose completeness is False, and
                    # readiness is True
                    node_list = [
                        x for x in node_list if x.data._completeness == False and x.data._ready]
    ...
    def belief_state_repair(
        ...
        for key in old_effect_summary:
            if key in new_effect_summary:
                ...
                key_term = key.split("/")
                belief_state_repair_summary[key] = db.get_reverse_attribute_value(
                    key_term[0], key_term[1], old_effect_summary[key]["value"])
